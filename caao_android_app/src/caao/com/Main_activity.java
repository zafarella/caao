/**
 * University of Eastern Finland Computer science department 
...
public class Main_activity extends TabActivity {
	...
	spec = TheTabs.newTabSpec("Calendar_Activity").setIndicator(
		tab_titles[0]).setContent(intent);
	TheTabs.addTab(spec);

	// Event notifications --------------------------------------------
	intent = new Intent().setClass(this, Event_Notification_Activity.class);
	spec = TheTabs.newTabSpec("Event_Notification_Activity").setIndicator(
		tab_titles[1]).setContent(intent);
	TheTabs.addTab(spec);

	// List of plants --------------------------------------------
	// Create an Intent to launch an Activity for the tab
	intent = new Intent().setClass(this, Plant_list_Activity.class);
	spec = TheTabs.newTabSpec("Plant_list_Activity").setIndicator(
		tab_titles[2]).setContent(intent);
	TheTabs.addTab(spec);
    ...
    // -----------------------------------------------------------------------------------
    /**
     * The menu handler method. The menu items id's can reveal the meaning of the items
     * 
     * @param item
     *            MenuItem
     * @return boolean
